...
def save_excel(results_csv, well_map, plate_name):
    ...
    # start writing with pandas + xlsxwriter engine
    with pd.ExcelWriter(out_file, engine='xlsxwriter') as writer:
        workbook = writer.book

        # Predefine palettes (you can adapt)
        base_color_hex = [
            "#1f77b4", "#ff7f0e", "#2ca02c", "#d62728",
            "#9467bd", "#8c564b", "#e377c2", "#7f7f7f"
        ]

        # iterate main groups -> one sheet each
        for main_group, conds in sorted(groups.items()):
            # safe sheet name
            sheet_name = str(main_group)[:31]
            # create empty dataframe to write data blocks in order
            ws = workbook.add_worksheet(sheet_name)
            # we'll manually write with worksheet to control positions
            row_cursor = 0
            ...
            # build subgroups mapping: subgroup -> list of (cond, conc)
            subgroups = {}
            for cond in conds:
                _, subgroup, conc = parsed[cond]
                subgroups.setdefault(subgroup, []).append((cond, conc))
            # sort concentrations within each subgroup
            for sub in subgroups:
                subgroups[sub].sort(key=lambda x: (x[1] is None, x[1]))  # None last

            # For deterministic colors per subgroup
            for s_idx, (subgroup, cond_list) in enumerate(sorted(subgroups.items())):
                base_color = base_color_hex[s_idx % len(base_color_hex)]
                # create gradient for this subgroup according to number of conditions
                n = max(1, len(cond_list))
                for i, (cond, conc) in enumerate(cond_list):
                    # color gradient: interpolate towards black for higher concentration
                    # convert hex to RGB
                    def hex_to_rgb(h):
                        h = h.lstrip('#')
                    ...
                    for _, r in dfc.iterrows():
                        ...
                        row_cursor += 1
                    end_row = row_cursor  # exclusive in our cursor usage

                    # Excel range strings (1-based)
                    # categories (x) -> column A, rows start_row..end_row
                    cat_range = f"='{sheet_name}'!$A${start_row}:${end_row}"  # placeholder fix below

                    # but XlsxWriter accepts the range as string like "=Sheet1!$A$2:$A$5"
                    cat = f"='{sheet_name}'!$A${start_row}:$A${end_row}"
                    vals = f"='{sheet_name}'!$B${start_row}:$B${end_row}"
                    errs = f"='{sheet_name}'!$C${start_row}:$C${end_row}"

                    # add series with custom error bars (std)
                    chart.add_series({
                        'name':       cond,
                        'categories': cat,
                        'values':     vals,
                    ...
                    # leave one blank row between conditions
                    row_cursor += 1

            # After adding subgroup/test series, add control groups appended at bottom
            for ctrl in control_groups:
                ...
                # choose light distinct control colors (pale palette)
                ...
                chart.add_series({
                    ...
                    'line': {'color': control_color, 'width': 2.5, 'dash_type': 'dash'},
                    'marker': {'type': 'square', 'size': 6}
                })

            # write chart to worksheet
            # place chart top-left at column F row 2 (zero-based indices)
            ws.insert_chart('F2', chart, {'x_scale': 1.4, 'y_scale': 1.1})

        # Save workbook (context manager auto-saves)
    return True
